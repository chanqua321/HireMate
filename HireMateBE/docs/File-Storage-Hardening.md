...
Production on a single Linux server: set `FileStorage__Provider=PersistentVolume` and `FileStorage__RootPath=/data/cv`. The AWS Compose file mounts a named Docker volume there. Startup fails unless this exact directory is a separate mount in `/proc/self/mountinfo`; an ordinary container directory is rejected. This survives container replacement, but not server or volume deletion. Back up database and CV volume together. This provider does not support scaling across multiple servers. Test actual read/write/delete and restart persistence on the target server before release.
